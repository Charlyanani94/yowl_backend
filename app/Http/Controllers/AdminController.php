...
class AdminController extends Controller
{
    ...
    public function updateUser(Request $request, $userId)
    {
        try {
            ...
            \Log::info("Admin {$request->user()->id} a mis à jour l'utilisateur {$userId}.");

            return $this->successResponse(['user' => $user->fresh()], 'Utilisateur mis à jour avec succès');
        } catch (ValidationException $e) {
            return $this->errorResponse($e->errors(), 'Erreur de validation', 422);
        } catch (\Exception $e) {
            return $this->errorResponse($e->getMessage(), 'Erreur lors de la mise à jour de l\'utilisateur', 500);
        }
    }
    ...
    public function deleteUser(Request $request, $userId)
    {
        try {
            $user = User::findOrFail($userId);
            // Empêcher suppression de soi-même admin courant
            if ($user->id === $request->user()->id) {
                return $this->errorResponse('Impossible de supprimer votre propre compte', 'Action non autorisée', 403);
            }

            // Optionnel: gestion cascade (soft delete ou detach). Ici suppression simple.
            $user->delete();
            \Log::warning("Admin {$request->user()->id} a supprimé l'utilisateur {$userId}.");
            return $this->successResponse(null, 'Utilisateur supprimé avec succès');
        } catch (\Exception $e) {
            return $this->errorResponse($e->getMessage(), 'Erreur lors de la suppression de l\'utilisateur', 500);
        }
    }
