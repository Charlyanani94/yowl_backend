...
class ReportController extends Controller
{
    ...
    public function reportPost(Request $request, $postId)
    {
        try {
            ...
            // Création du le signalement (sans actions automatiques)
            ...
            // Les actions automatiques se déclencheront seulement si l'admin accepte le signalement
            
            return $this->successResponse([
                'report' => $report,
                'message' => 'Signalement soumis pour examen par un administrateur'
            ], 'Signalement créé avec succès, en attente de validation', 201);
    ...
    /**
     * @OA\Patch(
     *     path="/admin/reports/{report}/process",
     *     tags={"Admin"},
     *     summary="Accepter ou rejeter un signalement",
     *     description="Traiter un signalement en l'acceptant ou le rejetant. Si accepté, déclenche les actions automatiques selon le nombre de signalements acceptés.",
     *     security={{"sanctum":{}}},
     *     @OA\Parameter(
     *         name="report",
     *         in="path",
     ...
     *         @OA\JsonContent(
     *             required={"decision"},
     *             @OA\Property(property="decision", type="string", enum={"accepted","rejected"}, example="accepted", description="Décision de l'admin sur le signalement"),
     *             @OA\Property(property="admin_note", type="string", nullable=true, example="Contenu effectivement inapproprié", description="Note administrative (optionnel, max 500 caractères)")
     *         )
     *     ),
     *     @OA\Response(
     *         response=200,
     ...
     *             @OA\Property(property="data", type="object",
     *                 @OA\Property(property="report", ref="#/components/schemas/Report"),
     *                 @OA\Property(property="accepted_reports_count", type="integer", example=3),
     *                 @OA\Property(property="actions_triggered", type="array", @OA\Items(type="string"), example={"warning_email_sent"})
     *             ),
     *             @OA\Property(property="message", type="string", example="Signalement accepté et actions automatiques déclenchées")
     *         )
     *     )
     * )
     */
    public function processReport(Request $request, $reportId)
    {
        try {
            $validated = $request->validate([
            ...
            // Permettre la modification même si déjà traité
            // Pas de vérification du statut - l'admin peut toujours modifier sa décision
            ...
            if ($validated['decision'] === Report::STATUS_RESOLVED) {
                $acceptedReportsCount = $this->handleAcceptedReport($report, $actionsTriggered);
            }
            ...
            return $this->successResponse([
                'report' => $report->load('processedBy'),
                'accepted_reports_count' => $acceptedReportsCount,
                'actions_triggered' => $actionsTriggered
            ], $validated['decision'] === Report::STATUS_RESOLVED ? 
                'Signalement résolu et actions automatiques déclenchées' : 
                'Signalement rejeté');
    ...
    /**
     * Gérer un signalement accepté - déclenche les actions automatiques si nécessaire
     */
    private function handleAcceptedReport($report, &$actionsTriggered)
    {
        // Compter SEULEMENT les signalements résolus pour ce post
        $acceptedReportsCount = Report::where('post_id', $report->post_id)
        ...
        // Actions automatiques basées sur signalements VALIDÉS uniquement
        if ($acceptedReportsCount == 3) {
            $this->sendWarningEmail($postOwner, $post, $acceptedReportsCount);
            $actionsTriggered[] = 'warning_email_sent';
            \Log::info("Avertissement envoyé à l'utilisateur {$postOwner->id} pour 3 signalements acceptés");
        } elseif ($acceptedReportsCount == 5) {
            $this->sendWarningEmail($postOwner, $post, $acceptedReportsCount);
            $this->deactivateUser($postOwner);
            $actionsTriggered[] = 'warning_email_sent';
            $actionsTriggered[] = 'account_deactivated';
            \Log::warning("Utilisateur {$postOwner->id} désactivé pour 5 signalements acceptés");
        }
    ...
    /**
     * Notifier le signaleur du résultat de son signalement
     */
    private function notifyReporter($report)
    {
        try {
            ...
            if ($report->status === Report::STATUS_RESOLVED) {
                $mailService->sendReportAcceptedNotification($report->reporter, $report);
            } elseif ($report->status === Report::STATUS_REJECTED) {
                $mailService->sendReportRejectedNotification($report->reporter, $report);
            }
            
        } catch (\Exception $e) {
            \Log::error("Erreur notification signaleur", [
                'report_id' => $report->id,
    ...
    {
        // Redirigervers la nouvelle méthode
        return $this->processReport($request, $reportId);
    }
}